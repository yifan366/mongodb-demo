...
import com.zhongtai.tools.Jsons;
...
public final class RedisManager {
    ...
    public void init() {
        ...
        Logs.Console.info("init redis...");
        List<RedisNode> nodes = Conf.mongodb.redis.nodes;
        List<RedisServer> redisServerList = new ArrayList<>();
        for(int i=0;i<nodes.size();i++) {
            RedisServer redisServer = new RedisServer(nodes.get(i).getHost(),nodes.get(i).getPort());
            redisServerList.add(redisServer);
        }

        factory.setPassword(Conf.mongodb.redis.password);
        factory.setPoolConfig(Conf.mongodb.redis.getPoolConfig(JedisPoolConfig.class));
        factory.setRedisServers(redisServerList);
        factory.setCluster(Conf.mongodb.redis.isCluster());
        factory.init();
        test();
